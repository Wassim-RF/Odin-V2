...
<body class="w-full min-h-screen bg-slate-50 flex flex-col">
    ...
    <main class="w-full flex flex-col gap-10 p-[5%]">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div class="flex bg-slate-200/60 p-1.5 rounded-2xl w-fit border border-slate-100 shadow-sm gap-2.5">
                    ...
                    <div class="group relative bg-white border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] rounded-4xl p-6 hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300">
                        <div class="absolute -top-3 left-6 z-30">
                            <span class="bg-indigo-600 text-white text-[10px] font-black px-4 py-1.5 rounded-full shadow-lg shadow-indigo-200 uppercase tracking-widest">
                                De: {{ \App\Models\User::find($link->pivot->sender_id)->name ?? 'Unknown' }}
                            </span>
                        </div>
                        ...
                        <div class="space-y-2">
                            <h3 class="text-lg font-black text-slate-900 truncate pr-10">{{ $link->title }}</h3>
                            <p class="text-[13px] font-bold text-slate-400 truncate tracking-tight">{{ $link->url }}</p>
                        </div>
                    ...
                    <div class="group relative bg-white border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] rounded-4xl p-6 hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300">
                        ...
                        <div class="absolute -top-3 left-6 z-30">
                            <span class="bg-emerald-500 text-white text-[10px] font-black px-4 py-1.5 rounded-full shadow-lg shadow-emerald-100 uppercase tracking-widest">
                                {{-- À: {{ \App\Models\User::find($link->pivot->user_id)->name ?? 'Unknown' }} --}}
                            </span>
                        </div>
                        ...
                        <div class="space-y-2">
                            <h3 class="text-lg font-black text-slate-900 truncate pr-10">{{ $link->name }}</h3>
                            <p class="text-[13px] font-bold text-slate-400 truncate tracking-tight">{{ $link->url }}</p>
                        </div>

                        <a href="{{ $link->url }}" target="_blank" class="mt-6 flex items-center justify-between py-3 border-t border-slate-50 group-hover:border-emerald-50 text-[#1B294B] font-black text-xs uppercase tracking-widest hover:text-emerald-600 transition-all">
                            <span>Visiter le lien</span>
                            <span>→</span>
                        </a>
                    </div>
                @empty
                    <p>Aucun lien envoiyee</p>
                @endforelse
