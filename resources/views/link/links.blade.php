...
<body class="w-full min-h-screen bg-slate-50 flex flex-col">
    ...
    <div class="absolute top-4 right-4 z-10">
        <form action="#" method="POST">
            ...
            <button title="Favoris" type="submit" 
                class="p-1.5 rounded-lg transition-all border {{ $link->is_favorite ? 'bg-red-50 border-red-100 text-red-500' : 'bg-slate-50 border-gray-100 text-slate-400 hover:text-red-500' }}">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="{{ $link->is_favorite ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <div class="absolute top-14 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    <div class="space-y-1">
        <h3 class="text-lg font-bold text-[#0F172A] tracking-tight truncate pr-8">{{ $link->title }}</h3>
